...
export function AdminIpWhitelist() {
  ...
  /** 401 nghĩa là apiFetch đã thử refresh và thất bại: phiên đã chết thật. */
  ...
  /** Nạp lại theo yêu cầu (nút "Thử lại"), nên có bật cờ loading. */
  ...
  // Lần nạp đầu: cờ `active` bỏ kết quả về muộn sau khi component đã rời khỏi
  // màn hình, thứ mà `load` (chạy do người dùng bấm) không cần tới.
  useEffect(() => {
    let active = true;

    fetchAdmins()
      .then((result) => {
        if (active) setAdmins(result.data);
      })
      .catch((failure: unknown) => {
        if (active) handleLoadFailure(failure);
      })
      .finally(() => {
  ...
  const openEditor = async (admin: AdminResponse) => {
    ...
    setLockoutIp(null);
    try {
      const detail = await fetchAdmin(admin.id);
      setEditor({
        admin: detail,
        name: detail.name,
        email: detail.email,
  ...
  const save = async (
    ...
    setLockoutIp(null);
    try {
      const updated = await updateAdmin(editor.admin.id, {
        name: editor.name,
        email: editor.email,
