...
export class KycStateMachine {
  planOpening(latest: KycSubmission | null): KycOpeningPlan {
    if (!latest) {
      return {
        kind: 'create',
        attemptNo: 1,
  ...
  requireEditable(latest: KycSubmission | null): KycSubmission {
  ...
  submit(
    ...
    this.assertTransition(submission.status, EKycStatus.PENDING, actor);
  ...
  review(
    ...
    this.assertTransition(submission.status, command.status, ERole.ADMIN);
