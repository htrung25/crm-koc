...
APP_ENV_FILE="$DIR/.env" STACK="$STACK" docker compose --env-file "$ROOT/api-$STACK/.env.deploy" -f "$ROOT/api-$STACK/compose.api.base.yml" -f "$ROOT/api-$STACK/compose.api.$STACK.yml" stop api || true
docker compose --env-file "$ROOT/web-$STACK/.env.deploy" -f "$ROOT/web-$STACK/compose.web.base.yml" -f "$ROOT/web-$STACK/compose.web.$STACK.yml" stop web || true
...
echo "==> bật lại api"
APP_ENV_FILE="$DIR/.env" STACK="$STACK" docker compose --env-file "$ROOT/api-$STACK/.env.deploy" -f "$ROOT/api-$STACK/compose.api.base.yml" -f "$ROOT/api-$STACK/compose.api.$STACK.yml" up -d api || true
docker compose --env-file "$ROOT/web-$STACK/.env.deploy" -f "$ROOT/web-$STACK/compose.web.base.yml" -f "$ROOT/web-$STACK/compose.web.$STACK.yml" up -d web || true
